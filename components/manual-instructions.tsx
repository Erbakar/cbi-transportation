...
type SourceResult={manual?:Manual;decisions?:string[];error?:string;fields:Fields;containers:Fields[];locations?:Record<string,string>;hblRequired?:boolean;agentName?:string;houseBillNumber?:string;blReference?:string;payments?:PositionPayments;moveType?:string};
...
export function ManualInstructions({pausedPlatforms=[],actualParties,recordId,onDirtyChange,value,fields,containers,cargoLines,disabled,onSave}:Props){
 const initial=():Manual=>resolveInstruction({...structuredClone(emptyManual),...value,inttra:{...emptyInttra,...value?.inttra}},{fields:fields||{},hblRequired:value?.inttra?.houseBill?value.inttra.houseBill==='2':undefined}).manual;
 ...
 async function importSource(){
  ...
  try{
   ...
   const next=structuredClone(data.manual||m);next.inttra={...emptyInttra,...next.inttra};
   if(data.moveType)next.moveType=data.moveType;
   if(data.payments)setSourcePayments(data.payments);
 ...
 return <section className="manual-instructions">
  ...
  <details><summary>Otomatik hazırlanan bilgiler</summary>{resolveInstruction(m,{fields:fields||{},hblRequired:m.inttra?.houseBill?m.inttra.houseBill==='2':undefined}).decisions.map(text=><p key={text}>{text}</p>)}</details>
  ...
  <section className="freight-summary"><div className="freight-heading"><div><h4>Freight Charges</h4><p>Masraf dağılımını kontrol edin.</p></div><Button variant="outline" disabled={disabled} aria-expanded={editingCharges} onClick={()=>setEditingCharges(!editingCharges)}>{editingCharges?'Düzenlemeyi kapat':'Düzenle'}</Button></div>
  <div className="freight-table-wrap"><table className="freight-table"><thead><tr><th scope="col">Charge Type</th><th scope="col">Freight Term</th><th scope="col">Payer</th></tr></thead><tbody>{m.charges.map((row,index)=><tr key={index}><th scope="row">{m.chargeMode==='all'?'All Charges':chargeTypes.find(o=>o.value===row.chargeType)?.label||'Masraf türü eksik'}</th><td>{row.freightTerm||'Seçim gerekli'}</td><td>{payerTypes.find(o=>o.value===row.payer)?.label||'Seçim gerekli'}{row.paymentLocation&&<small>Ödeme yeri: {row.paymentLocation.split(':').slice(2).join(':')||row.paymentLocation}</small>}</td></tr>)}</tbody></table>{!m.charges.length&&<p className="freight-empty">Masrafları hazırlamak için T-MAXX yük bilgilerini getirin.</p>}</div>
  {editingCharges&&<div className="freight-editor"><label>Ödeme kapsamı<select disabled={disabled} value={m.chargeMode} onChange={event=>set({...m,chargeMode:event.target.value as Manual['chargeMode'],charges:event.target.value==='all'?[m.charges[0]||{chargeType:'',freightTerm:'',payer:'',paymentLocation:''}]:m.charges})}><option value="">Seçin</option><option value="all">All Charges</option><option value="individual">Individual Charges</option></select></label>
  ...
  <div className="detail-actions"><Button variant="outline" disabled={disabled||m.charges.length>=20||(m.chargeMode==='all'&&m.charges.length===1)} onClick={()=>set({...m,charges:[...m.charges,{chargeType:'',freightTerm:'',payer:'',paymentLocation:''}]})}>Masraf satırı ekle</Button></div></div>}</section>
  <div className="detail-actions form-save"><Button disabled={disabled||importing} onClick={()=>onSave(m)}>Kaydet ve kontrol et</Button></div>
 </section>;
}
