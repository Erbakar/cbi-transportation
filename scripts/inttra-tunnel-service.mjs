...
const relayKey=fs.readFileSync(process.env.RELAY_KEY_FILE||path.join(root,'.private/inttra-relay/key'),'utf8').trim();
let stopping=false,updater,timer,pending='',saved='',healthFailures=0;
...
function stop(){stopping=true;clearTimeout(timer);clearInterval(healthTimer);tunnel.kill('SIGTERM');updater?.kill('SIGTERM');setTimeout(()=>process.exit(0),1000).unref();}
