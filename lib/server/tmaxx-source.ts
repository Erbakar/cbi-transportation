...
export async function tmaxxSource(owner:string,reference:string,source:Extraction){
 ...
 return {ex,goodId:goods[0].id,positionId:positions[0].id,reference,blankContainers:blank,houseBillNumber:typeof good.hblNo==='string'?good.hblNo:'',blReference:typeof position.masterBlNo==='string'?position.masterBlNo:'',agentName:selected.abroadAgent?.name||'',payments:{freight:position.freightPayment,local:position.localPayment,discharge:position.dischargePayment},ports:{loadPort:(position.fromPort as {name?:string})?.name,dischargePort:(position.toPort as {name?:string})?.name,origin:(good.placeOfReceiptCity as {name?:string})?.name,destination:(good.placeOfDeliveryCity as {name?:string})?.name}};
}
